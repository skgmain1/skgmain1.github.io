document.addEventListener('DOMContentLoaded', () => {
    ...
    const initStaggerAnimation = () => {
        ...
        Cache.staggerElements.forEach(wrapper => {
            ...
            wrapper.querySelectorAll('.char').forEach((char) => {
               ...
               char.style.transitionDelay = `calc(${i} * 0.05s)`;
           });
    ...
    const setLanguage = (lang) => {
        ...
        Cache.translatables.forEach(el => {
            ...
            } else if (el.matches('.stat-label, .price-label, .popular-badge, .faq-question > span:first-child, .status-item-name, .status-item-value, .modal-footer .btn-secondary')) {
                el.textContent = translation;
            } else if (el.matches('.modal-title, .payment-methods, h3:not(.modal-title), .product-title > span[data-translate-key]')) {
                 el.textContent = translation;
            } else if (el.matches('.modal-body > p, .modal-body li')) {
                el.innerHTML = translation;
            } else if (el.matches('h1, h2, p:not(.hero-subtitle):not(.modal-body p), .faq-answer p, .script-hub-card p, .script-hub-card li, .feature-card p')) {
