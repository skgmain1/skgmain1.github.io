document.addEventListener('DOMContentLoaded', () => {
        ...
        Cache.translatables.forEach(el => {
            ...
            if (el.classList.contains('nav-link')) {
                ...
                el.innerHTML = translation;
                 ...
                 if (!el.classList.contains('stagger-letters') && !el.classList.contains('logo')) { el.textContent = translation; } // Avoid translating logo text directly here, CSS handles it
             } else if (el.classList.contains('logo') && key === 'logo_text') {
                 // If we need dynamic text for the logo, handle it carefully, otherwise CSS styling is preferred
                 // For now, just setting the data attribute if needed
    ...
    const fetchAndUpdateStatus = () => {
        ...
        if (Cache.statusBox.indicator && Cache.statusBox.title) {
            ...
            const titleTranslationKey = `status_main_title_operational`;
            Cache.statusBox.title.textContent = getTranslation(titleTranslationKey) || getTranslation('status_main_title_operational'); // Fallback just in case
        ...
        if (Cache.statusBox.list) {
             ...
             simulatedStatus.services.forEach(service => {
                 ...
                 const statusKey = `status_operational`;
                 valueSpan.className = `status-item-value operational`; 
            ...
            serviceItems?.forEach((item, index) => {
                 ...
                 const statusKey = `status_operational`;
                 valueEl.className = `status-item-value operational`;
